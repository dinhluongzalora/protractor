import {By as WebDriverBy, promise, WebDriver, WebElement} from 'selenium-webdriver';

let clientSideScripts: any = require('./clientsidescripts');


export class By extends WebDriverBy {
  constructor(using?: string, value?: string) { super('using', 'value'); }

  [name: string]: any;
  findElementsOverride:
      (driver: webdriver.WebDriver, using: WebElement,
       rootSelector: string) => promise.Promise<WebElement[]>;
  row: (index: number) => By;
  column: (index: string) => By;

  // browser.driver.findElement(By.binding('')); x
   ...
   */
  addLocator(name: string, script: Function|string, ...args: any[]): void {
    let by = new By();

    by.findElementsOverride = (
        driver: webdriver.WebDriver,
        using: WebElement, rootSelector: string
      ): promise.Promise<WebElement[]> => {

      let findElementArguments: any[] = [script];
      for (let i = 0; i < args.length; i++) {
        findElementArguments.push(args[i]);
      }
      findElementArguments.push(using);
      findElementArguments.push(rootSelector);

      return driver.findElements(
          By.js.apply(By, findElementArguments));
    };
   ...
   */
  binding(bindingDescriptor: string): By {
    let by = new By();
    by.findElementsOverride = (
        driver: webdriver.WebDriver, using: WebElement,
        rootSelector: string
   ...
   * @returns {Locator} location strategy
   */
  exactBinding(bindingDescriptor: string): By {
    let by = new By();
    by.findElementsOverride = (
        driver: webdriver.WebDriver, using: WebElement,
        rootSelector: string
  ...
  model(model: string): By {
    let by = new By();
    by.findElementsOverride = (
        driver: webdriver.WebDriver, using: WebElement, rootSelector: string
      ): promise.Promise<WebElement[]> => {
   ...
   * @returns {Locator} location strategy
   */
  buttonText(searchText: string): By {
    let by = new By();
    by.findElementsOverride = (
        driver: webdriver.WebDriver, using: WebElement, rootSelector: string
      ): promise.Promise<WebElement[]> => {
   ...
   * @returns {Locator} location strategy
   */
  partialButtonText(searchText: string): By {
    let by = new By();
    by.findElementsOverride = (
        driver: webdriver.WebDriver, using: WebElement, rootSelector: string
      ): promise.Promise<WebElement[]> => {
  ...
  private byRepeaterInner(exact: boolean, repeatDescriptor: string): By {
    let by = new By();
    ...
    by.row = (index: number): By => {
      let byRow = new By();
      byRow.findElementsOverride = (
          driver: webdriver.WebDriver, using: WebElement, rootSelector: string
        ): promise.Promise<WebElement[]> => {

        return driver.findElements(By.js(
            clientSideScripts.findRepeaterRows, repeatDescriptor,
            exact, index, using, rootSelector));
      };
      byRow.toString = (): string => {
        return name + '(' + repeatDescriptor + '").row("' + index + '")"';
      };
      byRow.column = (binding: string): By => {
        let byCol = new By();
        byCol.findElementsOverride = (
            driver: webdriver.WebDriver, using: WebElement, rootSelector: string
          ): promise.Promise<WebElement[]> => {
        ...
        byCol.toString = (): string => {
          return name + '("' + repeatDescriptor + '").row("' +
            index + '").column("' + binding + '")';
        };
        return byCol;
      };
      return byRow;
    };
    by.column = (binding: string): By => {
      let byCol = new By();
      byCol.findElementsOverride = (
          driver: webdriver.WebDriver, using: WebElement, rootSelector: string
        ): promise.Promise<WebElement[]> => {
   ...
   * @returns {Locator} location strategy
   */
  repeater(repeatDescriptor: string): By {
   ...
   * @returns {Locator} location strategy
   */
  exactRepeater(repeatDescriptor: string): By {
   ...
   * @returns {Locator} location strategy
   */
  cssContainingText(cssSelector: string, searchText: string): By {
    let by = new By();
    by.findElementsOverride = (
          driver: webdriver.WebDriver, using: WebElement, rootSelector: string
        ): promise.Promise<WebElement[]> => {
   ...
   * @returns {Locator} location strategy
   */
  options(optionsDescriptor: string): By {
    let by = new By();
    by.findElementsOverride = (
          driver: webdriver.WebDriver, using: WebElement, rootSelector: string
        ): promise.Promise<WebElement[]> => {
