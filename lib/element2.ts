...
import { By as WebDriverBy, WebDriver, WebElement, WebElementPromise, promise, error } from 'selenium-webdriver';
...
export class WebdriverWebElement {
  ...
  findElement: (subLocator: WebDriverBy) => promise.Promise<any>;
 ...
 * However, an ElementArrayFinder will not actually retrieve the elements untilNoSuchElementError
...
export class ElementArrayFinder extends WebdriverWebElement {
  constructor(public browser_: ProtractorBrowser, public getWebElements: () => promise.Promise<WebElement> = null, public locator_?: By | WebDriverBy, public actionResults_: promise.Promise<any> = null) {
    super();
   ...
   * @returns {ElementArrayFinder}
   */
  all(locator: By | WebDriverBy): ElementArrayFinder {
    ...
    let getWebElements = (): promise.Promise<WebElement[]> => {
      if (this.getWebElements === null) {
        ...
        return this.getWebElements().then((parentWebElements: WebElement[]) => {
          ...
          // construct a list of Promise<List<child_web_element>>
   ...
   */
  filter(filterFn: (finder: ElementFinder, val: number) => promise.Promise<WebElement>): ElementArrayFinder {
    let getWebElements = (): promise.Promise<WebElement[]> => {
          return this.getWebElements().then((parentWebElements: WebElement[]) => {
            let list = parentWebElements.map(
                (parentWebElement: WebElement, index: number): ElementArrayFinder => {
  ...
  count(): webdriver.promise.Promise<number> {
    return this.getWebElements().then(
        ...
        (err: Error) => {
          if (err.code == new error.NoSuchElementError().code()) {
            return 0;
          } else {
            throw err;
          }
   ...
   */
  locator(): By | WebDriverBy { return this.locator_; }
   ...
   * @private
   */
  applyAction_(actionFn: (value: WebElement, index: number, array: WebElement[]) => ElementArrayFinder): ElementArrayFinder {
    let callerError = new Error();
    let actionResults = this.getWebElements().then(
        (arr: WebElement[]): promise.Promise<ElementArrayFinder> => {
      return promise.all(arr.map(actionFn));
  ...
  //  */
  // asElementFinders_(): promise.Promise<ElementFinder> {
   ...
   */
  reduce<T>(reduceFn: (val1: number, finder: ElementFinder, val2: number, array: ElementFinder[]) => ElementFinder, initialValue: T): promise.Promise<T> {
